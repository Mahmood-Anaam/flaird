...
def vectorize(truth_dict, pred_dict, missing_default=0.5):
    ...
    scores = [
        (truth_dict[k]["label"], pred_dict.get(k, missing_default)["label"])
        for k in sorted(truth_dict)
    ]
...
def optimize_pred_scores(y_true, y_pred, max_steps=1000) -> float:
    ...
    min_offset = -(np.max(y_pred) - 0.49)
    max_offset = 0.51 - np.min(y_pred)
    best_offset = 0.0
    ...
    for offset in np.linspace(min_offset, max_offset, max_steps):
        ...
        if score > best_score:
            ...
            best_offset = offset
            score_updated = True
        elif score_updated and score < best_score - 0.01:
            break
    return best_offset
...
def main(answer_file, truth_file, output_dir, outfile_name):
    ...
    with open(output_dir / outfile_name, "w") as f:
        f.write(jstr)
